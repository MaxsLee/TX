...
def get_tpai_features():
    ...
    features_encode_txt_0 = current_path+sig+'features_encode_0.txt'
    features_encode_txt_1 = current_path + sig + 'features_encode_1.txt'
    if os.path.exists(features_encode_txt_0):
        os.remove(features_encode_txt_0)
    if os.path.exists(features_encode_txt_1):
        os.remove(features_encode_txt_1)
    ...
    # write features_encode_1.txt
    with codecs.open(features_encode_txt_1, mode='w') as write_features_txt_1:
        # write features_encode_0.txt
        with codecs.open(features_encode_txt_0, mode='w') as write_features_txt_0:
            # read train.csv
            with codecs.open(train_file, mode='r') as read_train:
                for line in islice(read_train, 1, None):
                    ...
                    if int(label) == 0:
                        write_features_txt_0.write(record+'\n')
                    elif int(label) == 1:
                        write_features_txt_1.write(record+'\n')
                    # list_a = record.split(',')
